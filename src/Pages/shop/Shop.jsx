...
import useMedicines from "../../Hooks/medicine/UseMedicine";
...
const Shop = () => {
  ...
  const addToCartMutation = useMutation({
    mutationFn: async (cartItem) => {
      return await axiosPublic.post("/carts", cartItem);
    },
    onSuccess: () => {
  ...
  const existing = await cart?.find(item => item.id === medicine.id);
  if (existing) {
    Swal.fire("Already in cart", `${medicine.name} is already added.`, "info");
    return;
  }
    const cartItem = {
      ...medicine,
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
      {medicines.map((medicine) => (
        <MedicineCard
          key={medicine._id}
          medicine={medicine}
          onView={() => setSelectedMedicine(medicine)}
      ...
      {selectedMedicine && (
        <MedicineModal
          ...
          onAdd={() => handleAddToCart(selectedMedicine)}
        />
      )}
    </div>
    </div>
